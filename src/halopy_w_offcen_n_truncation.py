...
class halo(constants):
    ...
    def __init__(self,log_mtot, con_par, omg_m=0.3, rtrunc=None, Roff=None, beta=None, Rmin=0.0001, Rmax=5, Rbins=80):
        ...
        #apply truncation to the halo profile if rtrunc is not None as a hard truncation for projecting the nfw profile. To model the subhalo lensing signal. 
        self.rtrunc = rtrunc
    ...
    def esd_nfw(self,r):
        ...
        if np.isscalar(r):
            r = np.array([r])
        # add both truncation and off centering effects individually as well as together. If not present, then do the usual analytical computation.
        
        if self.rtrunc is not None:
            sig = self.num_avg_sigma(r, self.nfw, 0) - self.num_sigma(r, self.nfw)
        elif self.Roff is None:
    ...
    def num_sigma(self, Rarr, func):
        ...
        # add the trucation here as well if rtrunc is not None. This will be a hard truncation for the projection of the nfw profile. To model the subhalo lensing signal.
        if self.rtrunc is not None:
             func = lambda r : func(r) * (r < self.rtrunc)
...
if __name__ == "__main__":
    #write a test case for checking the truncation
    hp = halo(15, 4, omg_m=0.3, rtrunc=0.1)
