...
class weakpipe():
    def __init__(self,H0 = 100, Om0 = 0.25, Ob0 = 0.044, Tcmb0 = 2.7255, Neff = 3.046, sigma8 = 0.8, ns = 0.95, Rmin=0.1, Rmax=1.0, Nbins=10, Njacks=20, outputfilename='dsigma.dat'):
    ...
    def initialize_arrays(self):
        ...
        self.sumd_dsigmat_num              = np.zeros(self.nbins * self.njacks)
        self.sumd_dsigmatsq_num            = np.zeros(self.nbins * self.njacks)
        self.sumd_dsigmax_num              = np.zeros(self.nbins * self.njacks)
        self.sumd_dsigmaxsq_num            = np.zeros(self.nbins * self.njacks)

        self.r90_sumd_dsigmat_num          = np.zeros(self.nbins * self.njacks)
        self.r90_sumd_dsigmatsq_num        = np.zeros(self.nbins * self.njacks)
        self.r90_sumd_dsigmax_num          = np.zeros(self.nbins * self.njacks)
        self.r90_sumd_dsigmaxsq_num        = np.zeros(self.nbins * self.njacks)


        self.sumdwls                       = np.zeros(self.nbins * self.njacks)
        self.r90_sumdwls                   = np.zeros(self.nbins * self.njacks)
        self.pair_counts                   = np.zeros(self.nbins * self.njacks)
        ...
        for ljk in jkidxs:
            for jk in range(self.njacks):
                ...
                for rb in range(self.nbins):
                    ...
                    self.sumd_dsigmat_num          [jk*self.nbins + rb]     +=sum((w_ls * etan_obs * sigma_crit)[idx])
                    self.sumd_dsigmatsq_num        [jk*self.nbins + rb]     +=sum(((w_ls* etan_obs * sigma_crit)**2)[idx])
                    self.sumd_dsigmax_num          [jk*self.nbins + rb]     +=sum((w_ls * ex_obs * sigma_crit)[idx])
                    self.sumd_dsigmaxsq_num        [jk*self.nbins + rb]     +=sum(((w_ls* ex_obs * sigma_crit)**2)[idx])
                    self.sumdwls                   [jk*self.nbins + rb]     +=sum(w_ls[idx])


                    self.r90_sumd_dsigmat_num      [jk*self.nbins + rb]     +=sum((w_ls * r90_etan_obs * sigma_crit)[idx])      
                    self.r90_sumd_dsigmatsq_num    [jk*self.nbins + rb]     +=sum(((w_ls* r90_etan_obs * sigma_crit)**2)[idx])  
                    self.r90_sumd_dsigmax_num      [jk*self.nbins + rb]     +=sum((w_ls * r90_ex_obs * sigma_crit)[idx])      
                    self.r90_sumd_dsigmaxsq_num    [jk*self.nbins + rb]     +=sum(((w_ls* r90_ex_obs * sigma_crit)**2)[idx])  
                    self.r90_sumdwls               [jk*self.nbins + rb]     +=sum(w_ls[idx])

                    self.pair_counts               [jk*self.nbins + rb]     +=sum(idx)
    ...
    def write2file(self):
        ...
        for jk in range(self.njacks):
            for i in range(self.nbins):
                ...
                dsig                =   self.sumd_dsigmat_num[jk*self.nbins + i]*1.0/self.sumdwls[jk*self.nbins + i]
                #sig_dsigt           =   np.sqrt(self.sumd_dsigmatsq_num[jk*self.nbins + i]*1.0/self.sumdwls[jk*self.nbins + i] - (self.sumd_dsigmat_num[jk*self.nbins + i]*1.0/self.sumdwls[jk*self.nbins + i])**2 )
                SN_Errdsigt         =   np.sqrt(self.sumd_dsigmatsq_num[jk*self.nbins + i])*1.0/self.sumdwls[jk*self.nbins + i]
                dsigx               =   self.sumd_dsigmax_num[jk*self.nbins + i]*1.0/self.sumdwls[jk*self.nbins + i]
                #sig_dsigx           =   np.sqrt(self.sumd_dsigmaxsq_num[jk*self.nbins + i]*1.0/self.sumdwls[jk*self.nbins + i] - (self.sumd_dsigmax_num[jk*self.nbins + i]*1.0/self.sumdwls[jk*self.nbins + i])**2 )
                SN_Errdsigx         =   np.sqrt(self.sumd_dsigmaxsq_num[jk*self.nbins + i])*1.0/self.sumdwls[jk*self.nbins + i]


                r90_dsig            =   self.r90_sumd_dsigmat_num[jk*self.nbins + i]*1.0/self.r90_sumdwls[jk*self.nbins + i]
                #r90_sig_dsigt       =   np.sqrt(self.r90_sumd_dsigmatsq_num[jk*self.nbins + i]*1.0/self.r90_sumdwls[jk*self.nbins + i] - (self.sumd_dsigmat_num[jk*self.nbins + i]*1.0/self.r90_sumdwls[jk*self.nbins + i])**2 )
                r90_SN_Errdsigt     =   np.sqrt(self.r90_sumd_dsigmatsq_num[jk*self.nbins + i])*1.0/self.r90_sumdwls[jk*self.nbins + i]
                r90_dsigx           =   self.r90_sumd_dsigmax_num[jk*self.nbins + i]*1.0/self.r90_sumdwls[jk*self.nbins + i]
                #r90_sig_dsigx       =   np.sqrt(self.r90_sumd_dsigmaxsq_num[jk*self.nbins + i]*1.0/self.r90_sumdwls[jk*self.nbins + i] - (self.sumd_dsigmax_num[jk*self.nbins + i]*1.0/self.r90_sumdwls[jk*self.nbins + i])**2 )
                r90_SN_Errdsigx     =   np.sqrt(self.r90_sumd_dsigmaxsq_num[jk*self.nbins + i])*1.0/self.r90_sumdwls[jk*self.nbins + i]
