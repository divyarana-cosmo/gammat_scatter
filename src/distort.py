...
class simshear():
    ...
    #def get_sigma_crit_inv(self, lzred, szred):
        ...
        sigm_crit_inv = sigm_crit_inv * 1e12*4*np.pi*gee*1.0/cee**2  # multipling with 1e12 to get the in measurement in the pc-2
    ...
    def _interp_get_sigma_crit_inv(self, szred):
        ...
        xx = np.linspace(0, szred, 100)
        yy = self._get_sigma_crit_inv(xx, szred)
        #yy = self._get_sigma_crit_inv(lzred, xx)
        return interp1d(xx, yy, kind='cubic')

    def _get_esd(self, logmstel,log_re,logmh, lconc, proj_sep):
        'provides the esd and sigma in pc-2 units'
        self.hp           = halo(logmh, lconc, omg_m=self.omg_m)
        self.stel         = stellar(logmstel, log_re)
        ...
        return esd_s/1e12, esd_dm/1e12, sigma_s/1e12, sigma_dm/1e12 



    def _get_g(self, logmstel,log_re, logmh, lconc, lzred, szred, proj_sep):
        ...
        #    self.get_sigma_crit_inv =   self._get_sigma_crit_inv(lzred, szred)
        #else:
        #    self.get_sigma_crit_inv = self.interp_get_sigma_crit_inv(lzred) 

        self.get_sigma_crit_inv = self.interp_get_sigma_crit_inv(lzred) 
        esd_s, esd_dm, sigma_s, sigma_dm =  self._get_esd(logmstel,log_re, logmh, lconc, proj_sep)
        #considering only tangential shear and adding both contributions
        gamma_s     =   esd_s     * self.get_sigma_crit_inv 
        gamma_dm    =   esd_dm    * self.get_sigma_crit_inv
        kappa_s     =   sigma_s   * self.get_sigma_crit_inv
        kappa_dm    =   sigma_dm  * self.get_sigma_crit_inv
        return gamma_s, gamma_dm, kappa_s, kappa_dm

    def get_g(self, lra, ldec, lzred, logmstel, log_re, logmh, lconc, sra, sdec, szred):
        ...
        #considering only tangential shear and adding both contributions
        gamma_s, gamma_dm, kappa_s, kappa_dm = self._get_g(logmstel,log_re, logmh, lconc, lzred, szred, proj_sep)
        ...
        s_theta = np.clip(np.sqrt(1-c_theta**2), -1.0, 1.0)

        sflag = sflag & (np.abs(s_theta)>np.sin(np.pi/180 * 1/3600)) & (np.abs(kappa)<0.5) & (np.abs(g)<1.0)  #weak lensing flag and proximity flag
    ...
    def shear_src(self, lra, ldec, lzred, logmstel, log_re, logmh, lconc, sra, sdec, szred, se1, se2):
        ...
        g_1, g_2, etan, kappa, c_phi, s_phi, proj_sep, sflag, g_b, g_dm = self.get_g(lra, ldec, lzred, logmstel,log_re, logmh, lconc, sra, sdec, szred)
        g   = g_1 + 1j* g_2
        ...
        return np.real(e), np.imag(e), etan, kappa, proj_sep, sflag, g_b, g_dm, etan_obs, ex_obs

if __name__ == "__main__":
    ss = simshear()

    proj_sep = np.logspace(-2,0,10)
    
    logmstel = 12
    logmh     = 14
    #assigning the half-light radius - need to add a scatter later on
    llog_re     = (0.774 + 0.977 *(np.log10(10**logmstel / 0.7) - 11.4)) #check arxiv:1811.04934
    llog_re     = np.log10(10**llog_re * 0.7/1e3) #h-1 kpc to h-1 Mpc
        


    gamma_s, gamma_dm, kappa_s, kappa_dm = ss._get_g(logmstel=logmstel, log_re = llog_re,logmh=logmh, lconc=4, lzred=0.5, szred=1.0, proj_sep=proj_sep)
    etan = (gamma_s + gamma_dm)/(1 - (kappa_s + kappa_dm))
    invsigmacrit = ss._get_sigma_crit_inv(lzred=0.5, szred=1.0)

    esdstel = ss.stel.esd_deVaucouleurs(proj_sep)
    esdnfw = ss.hp.esd_nfw(proj_sep)

    plt.subplot(2,2,1)

    plt.plot(proj_sep, etan/invsigmacrit)
    plt.plot(proj_sep, esdstel/1e12)
    plt.plot(proj_sep, esdnfw/1e12)
    plt.plot(proj_sep, (esdstel + esdnfw)/1e12, '-k')
