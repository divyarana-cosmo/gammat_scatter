import sys
sys.path.append('/home/rana/github/ggl_aroundsource/src/')
...
def plt_data(config, outputfilename):
    ...
    llogmstel   = 10.0
    llogmh      = 12.0 #+ 0.0*llogmh
    lzred       = 0.2  #+ 0.0*lzred
    lconc       = 5.0  #concentration.concentration(1e14, '200m', lzred, model = 'diemer19')
    ...
    for i in range(len(rbins)):
        ...
        yyx[i]       = np.mean(dat[idx,4])
        yyerrx[i]    = (sum(idx) - 1)**0.5 * np.std(dat[idx,4])
