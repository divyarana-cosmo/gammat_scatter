...
def run_pipe(config, outputfile = 'gamma.dat', outputpairfile=None):
    ...
    lconc       = 5.0 + 0.0*llogmstel
    lzred       = 0.2 + 0.0*llogmstel
    ...
    sumdgammat_num_leq_r    = np.zeros(nbins)
    sumdgammatsq_num_leq_r  = np.zeros(nbins)
    sumdgammax_num_leq_r    = np.zeros(nbins)
    sumdgammaxsq_num_leq_r  = np.zeros(nbins)
    sumdwls_leq_r           = np.zeros(nbins)
    ...
    for igal in tqdm(range(Ngal)):
        ...
        for rb in range(nbins):
            ...
            sumdgammat_num_leq_r    [rb]     +=sum((w_ls * et)[idx1])
            sumdgammatsq_num_leq_r  [rb]     +=sum(((w_ls* et)**2)[idx1])
            sumdgammax_num_leq_r    [rb]     +=sum((w_ls * ex)[idx1])
            sumdgammaxsq_num_leq_r  [rb]     +=sum(((w_ls* ex)**2)[idx1])
            sumdwls_leq_r           [rb]     +=sum(w_ls[idx1])

    #exit()
    fout = open(outputfile, "w")
    fout.write("# 0:rmin/2+rmax/2 1:gammat 2:sig_gammat_leq_r 3:SN_Errgammat 4:gammax 5:sig_gammax_leq_r 6:SN_Errgammax 7:truegamma\n")

    for i in range(len(rbins[:-1])):
        rrmin = rbins[i]
        rrmax = rbins[i+1]
        fout.write("%le\t%le\t%le\t%le\t%le\t%le\t%le\t%le\n"%(rrmin/2.0+rrmax/2.0, sumdgammat_num[i]*1.0/sumdwls[i], np.sqrt(sumdgammatsq_num_leq_r[i]*1.0/sumdwls_leq_r[i] - (sumdgammat_num_leq_r[i]*1.0/sumdwls_leq_r[i])**2 ), np.sqrt(sumdgammatsq_num[i])*1.0/sumdwls[i], sumdgammax_num[i]*1.0/sumdwls[i], np.sqrt(sumdgammaxsq_num_leq_r[i]*1.0/sumdwls_leq_r[i] - (sumdgammax_num_leq_r[i]*1.0/sumdwls_leq_r[i])**2 ), np.sqrt(sumdgammaxsq_num[i])*1.0/sumdwls[i], sumdgammat_inp_num[i]*1.0/sumdwls[i]))
    fout.write("#OK")
    fout.close()
