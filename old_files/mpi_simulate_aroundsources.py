...
def run_pipe(config, outputfile = 'gamma.dat', outputpairfile=None, seljk=None):
    rmin        = config['Rmin'] 
    rmax        = config['Rmax'] 
    nbins       = config['Nbins']
    ...
    # taking only the lenses in one jackknife region
    idx         = lxjkreg == seljk
    lid         = lid      [idx]        
    lra         = lra      [idx] 
    ldec        = ldec     [idx] 
    ...
    if config['test_case']:
        ...
        llogmstel   = 12.0 + 0.0*llogmstel
        llogmh      = 14.0 + 0.0*llogmstel
        lconc       = 5.5 + 0.0*llogmstel
        lzred       = 0.3 + 0.0*llogmstel
    else:
    ...
    thetamax    = np.pi/2   #(90 - np.min(ldec))* np.pi/180
    thetamin    = 0.0       #(90 - np.max(ldec))* np.pi/180
    ramin       = 0.0       #np.min(lra)*np.pi/180
    ramax       = np.pi/2   #np.max(lra)*np.pi/180
    ...
    fout.write("# 0:rmin 1:rmax 2:sumdgammat_num 3:sumdgammatsq_num 4:sumdgammax_num 5:sumdgammaxsq_num 6:sumd_wls \n")

    for i in range(len(rbins[:-1])):
        rrmin = rbins[i]
        rrmax = rbins[i+1]

        fout.write("%le\t%le\t%le\t%le\t%le\t%le\t%le\n"%(rrmin, rrmax, sumdgammat_num[i], sumdgammatsq_num[i], sumdgammax_num[i], sumdgammaxsq_num[i], sumdwls[i]))
    fout.write("#OK")
    fout.close()
...
if __name__ == "__main__":
    ...
    for jk in range(config['lens']['Njacks']):
        ...
        output_filename = outputfilename + '_seljk_%d'%(jk)
        np.random.seed(args.seed)
        run_pipe(config, outputfile = output_filename, seljk=jk)           
