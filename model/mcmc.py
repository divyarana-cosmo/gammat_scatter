...
def lnprior(x):
    log_Mstel, log_Mh = x
    if 8<=log_Mstel<=13 and 9<=log_Mh<=16 :
        return 0.0
    return -np.inf
...
if __name__ == "__main__":
    ...
    with MPIPool() as pool:
        ...
        Ntotal = 4000
        if rot90:
            pos = runchain(Ntotal,sampler, '/net/dobbe/data2/model/%s/%s_Rpin_%s_Rpmax_%s_Burnfile_fixed_conc.dat_r90'%( outputdir, outputfilename.split('/')[-1], Rpin, Rpmax), p_0)
        else:
            pos = runchain(Ntotal,sampler, '/net/dobbe/data2/model/%s/%s_Rpin_%s_Rpmax_%s_Burnfile_fixed_conc.dat'%( outputdir, outputfilename.split('/')[-1], Rpin, Rpmax), p_0)
        sampler.reset()

        print("Running production...")
        Ntotal = 8000
        if rot90:
            pos = runchain(Ntotal,sampler, './%s/%s_Rpin_%s_Rpmax_%s_Chainfile_fixed_conc.dat_r90'%( outputdir, outputfilename.split('/')[-1], Rpin, Rpmax), pos)
        else:
            pos = runchain(Ntotal,sampler, './%s/%s_Rpin_%s_Rpmax_%s_Chainfile_fixed_conc.dat'%( outputdir, outputfilename.split('/')[-1], Rpin, Rpmax), pos)
