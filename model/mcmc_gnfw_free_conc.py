...
if __name__ == "__main__":
    ...
    with MPIPool() as pool:
        ...
        Ntotal = 4000
        if rot90:
            pos = runchain(Ntotal,sampler, './%s/%s_Rpin_%s_Rpmax_%s_Burnfile_free_conc_gnfw.dat_r90'%( outputdir, outputfilename.split('/')[-1], Rpin, Rpmax), p_0)
        else:
            pos = runchain(Ntotal,sampler, './%s/%s_Rpin_%s_Rpmax_%s_Burnfile_free_conc_gnfw.dat'%( outputdir, outputfilename.split('/')[-1], Rpin, Rpmax), p_0)
        sampler.reset()

        print("Running production...")
        Ntotal = 4000
        if rot90:
            pos = runchain(Ntotal,sampler, './%s/%s_Rpin_%s_Rpmax_%s_Chainfile_free_conc_gnfw.dat_r90'%( outputdir, outputfilename.split('/')[-1], Rpin, Rpmax), pos)
        else:
            pos = runchain(Ntotal,sampler, './%s/%s_Rpin_%s_Rpmax_%s_Chainfile_free_conc_gnfw.dat'%( outputdir, outputfilename.split('/')[-1], Rpin, Rpmax), pos)
