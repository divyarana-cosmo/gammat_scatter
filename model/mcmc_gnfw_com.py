...
def lnprior(x):
    alpha, logmh, conc, beta = x
    if 0.1<alpha<5 and 1e9<10**logmh<1e16 and 0.5<conc<20 and 0<beta<3:
        return 0.0 
    return -np.inf
...
if __name__ == "__main__":
    ...
    p_beta      =   np.random.uniform(0, 3, nwalkers)

    p_0         = np.transpose([p_alpha, p_logmh, p_conc, p_beta])
   ...
   # Initialize the sampler
    ...
    burnfile        =   './%s/burnfile_nfw_lmstelmin_%2.2f_lmstelmax_%2.2f.dat_full_desixeuclid_seed_%d_free_conc_gnfw_14kdr3'%(outputdir, logMmin, logMmax, args.seed)
    burnpredfile    =   './%s/burnpredfile_nfw_lmstelmin_%2.2f_lmstelmax_%2.2f.dat_full_desixeuclid_seed_%d_free_conc_gnfw_14kdr3'%(outputdir, logMmin, logMmax, args.seed)
    ...
    Ntotal = 8000
    chainfile = './%s/chainfile_nfw_lmstelmin_%2.2f_lmstelmax_%2.2f.dat_full_desixeuclid_seed_%d_free_conc_gnfw_14kdr3'%(outputdir, logMmin, logMmax, args.seed)
    predfile  = './%s/predfile_nfw_lmstelmin_%2.2f_lmstelmax_%2.2f.dat_full_desixeuclid_seed_%d_free_conc_gnfw_14kdr3'%(outputdir, logMmin, logMmax, args.seed)
